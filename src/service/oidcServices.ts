...
export const isRequireAuthentication = (user: User | null, isForce?: boolean): boolean =>
...
export const authenticateUser = (
    ...
    user: User | null = null
) => async (isForce: boolean = false, callbackPath: string | null = null) => {
    let oidcUser = user;
    if (!oidcUser) {
    ...
    if (isRequireSignin(oidcUser, isForce)) {
        ...
        try {
            ...
            if (numberAuthentication <= 1) {
                ...
                history && history.push(`/authentication/session-lost?path=${encodeURI(url)}`);
            }
        }
        userRequested = false;
    }
